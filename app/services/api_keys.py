...
class ApiKeyManager:
    ...
    def _schedule_usage_save(self):
        """合并高频 usage 写入，降低磁盘 I/O 压力。"""
        self._usage_dirty = True
        if self._usage_save_task and not self._usage_save_task.done():
            return
        self._usage_save_task = asyncio.create_task(self._flush_usage_loop())

    async def _flush_usage_loop(self):
        try:
            while True:
                await asyncio.sleep(self._usage_save_delay)
                if not self._usage_dirty:
                    break
                self._usage_dirty = False
                await self._save_usage_data()
        finally:
            self._usage_save_task = None
